...
class HealthCheck extends Tool {
   ...
   * @param configuration.properties.queue - queue name where to produce healthCheck updates
   * @param configuration.properties.topic - topic name where to publish healthCheck updates
  ...
  constructor(dependencies, configuration) {
    ...
    const that = this;
    this.file = validate(configuration.properties, {
      type: 'object',
      optional: true,
    ...
    this.logger.verbose(`File set to "${this.file}"`);
    let content = null;
    try {
      content = jsonfile.readFileSync(this.file);
    } catch (e) {
      this.logger.verbose(`File "${this.file}" not found, it will be generated on the first healthcheck update`);
    }
   ...
   * @param {string} data.reason - reason of the status
  ...
  update(data) {
    ...
    jsonfile.writeFile(this.file, this.healths, null, () => {
